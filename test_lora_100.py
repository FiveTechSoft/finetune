#!/usr/bin/env python3
"""
Test battery for LoRA-finetuned model.
Identical tests as test_baseline_100.py but hitting the LoRA server.
"""
...
RESULTS_FILE = Path("/home/fivetech/finetune/test_lora_100.json")
...
def query(prompt, timeout=1200):
    payload = {"model": MODEL, "prompt": prompt, "stream": False,
               "options": {"temperature": 0.2, "num_predict": 500, "top_p": 0.9}}
    try:
        t0 = time.time()
...
def save(results, meta):
    with open(RESULTS_FILE, "w") as f:
        json.dump({"model":"Qwen3.6-35B-A3B + LoRA","ts":datetime.now().isoformat(),**meta,"results":results}, f, indent=2, ensure_ascii=False)
...
print(f"{'='*60}")
print(f"LORA TEST BATTERY: {len(TESTS)} tests | LoRA server on port 11435")
...
for attempt in range(30):
    try:
        s = socket.create_connection(("localhost", 11435), timeout=2)
        s.close()
        break
    except:
        print(f"Waiting for LoRA server... (attempt {attempt+1})")
        time.sleep(10)
else:
    print("ERROR: LoRA server not available after 5 minutes")
    sys.exit(1)
...
for i, (tid, cat, name, prompt) in enumerate(TESTS, 1):
    ...
    if res.get("err"):
        ...
        fail_c += 1
        print(f"\r[{i:3d}/{len(TESTS)}] {tid} {name}... ERR: {res['err'][:60]}")
...
print(f"\n\n{'='*60}")
print(f"SUMMARY - LoRA Fine-tuned Model")
print(f"{'='*60}")
...
try:
    with open("/home/fivetech/finetune/test_baseline_100.json") as f:
        bl = json.load(f)
    print(f"\n{'='*60}")
    print(f"COMPARISON vs BASELINE (qwen3.6:35b)")
    print(f"{'='*60}")
    print(f"  Baseline: {bl['pass']}/{bl['pass']+bl['fail']} = {bl['rate']:.1f}%")
    print(f"  LoRA:     {pass_c}/{len(results)} = {pass_c/len(results)*100:.1f}%")
    delta = pass_c/len(results)*100 - bl['rate']
